...
def create_app(db_url=None):
    ...
    app.config["JWT_SECRET_KEY"] = "320904797155499156953493812259559955142" #secrets.SystemRandom().getrandbits(128) #os.getenv("keepitsecret")
    jwt = JWTManager(app)

    @jwt.token_in_blocklist_loader
    def check_if_token_in_blocklist(jwt_header, jwt_payload):
        token = jwt_payload["jti"]
        is_blocked = BlockList.query.filter_by(token=token).first()
        if is_blocked:
            return True
        else:
            return False
